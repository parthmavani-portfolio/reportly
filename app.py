...
@app.route("/")
def index():
    """Serve the main Reportly page. Looks for index.html in multiple locations."""
    for path in [
        os.path.join(BASE_DIR, "templates", "index.html"),  # standard Flask location
        os.path.join(BASE_DIR, "index.html"),                # root of project
    ]:
        if os.path.exists(path):
            return send_file(path)
    # Debug info if neither exists
    files = os.listdir(BASE_DIR)
    return f"""<html><body>
    <h1>Reportly - index.html not found</h1>
    <p>Looked in: templates/index.html and ./index.html</p>
    <p>Files in project root: {files}</p>
    <p>Fix: make sure index.html exists in your GitHub repo.</p>
    </body></html>""", 500
...
def health():
    ...
    return jsonify({
        ...
        "index_in_root": os.path.exists(os.path.join(BASE_DIR, "index.html")),
        "index_in_templates": os.path.exists(os.path.join(BASE_DIR, "templates", "index.html")),
    })
